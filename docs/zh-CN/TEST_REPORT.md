...
> Deno test runner 将 2 条框架收尾步骤计入总数，故 Deno 显示 54、Bun/Node 显示
> 52；三端业务 `it()` 用例一致，均为 0 失败。
...
`@dreamer/config` 三端（Deno/Bun/Node）全部通过：**54 / 52 / 52，0 失败** （Deno
较 Bun/Node 多 2 条为框架收尾步骤，业务 `it()` 用例一致为 52）。覆盖
